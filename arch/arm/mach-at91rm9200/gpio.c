...
static inline struct pio_controller __iomem *pin_to_controller(unsigned pin)
{
	pin -= PIN_BASE;
	pin /= 32;
	if (likely(pin < PQFP_GPIO_BANKS))//modified by wjzhe, BGA_GPIO_BANKS
...
int at91_set_multi_drive(unsigned pin, int is_on)
{
	...
	__raw_writel(mask, (is_on ? &pio->mder : &pio->mddr));
	return 0;
}
...
int gpio_direction_output(unsigned pin, int value)
{
	...
	if (!pio || !(__raw_readl(&pio->psr) & mask))
		return -EINVAL;
...
void __init at91_gpio_irq_setup(unsigned banks)
{
	...
	for (pioc = 0, pin = PIN_BASE, id = AT91C_ID_PIOA;
		...
		unsigned			i;
		/*
		 * can not set irq gpiob
		 * modified by wjzhe, 3/30 2007
		 */
		//if (id == AT91C_ID_PIOB)
		//	continue;
		/////////////////////////////////
