	...
	if (at91_cf_present(cf)) {
		...
		*sp = SS_DETECT | SS_3VCARD;
		...
		if (!vcc || at91_get_gpio_value(vcc))
			*sp |= SS_POWERON;
		s->irq.AssignedIRQ = cf->board->irq_pin;
	} else
		*sp = 0;
	return 0;
}
...
static int at91_cf_set_io_map(struct pcmcia_socket *s, struct pccard_io_map *io)
{
	...
	if (!(io->flags & (MAP_16BIT|MAP_AUTOSZ))) {
		csr |= AT91C_SMC2_DBW_8;
		pr_debug("%s: 8bit i/o bus\n", driver_name);
	} else {
		csr |= AT91C_SMC2_DBW_16;
		pr_debug("%s: 16bit i/o bus\n", driver_name);
	}
	AT91_SYS->EBI_SMC2_CSR[4] = csr;
...
static int __init at91_cf_probe(struct device *dev)
{
	...
	AT91_SYS->EBI_CSA |= AT91C_EBI_CS4A;
...
#define SM_RWH		(4 << 28)	/* hold time */
#define SM_RWS		(6 << 24)	/* setup time */
#define SM_TDF		(1 << 8)	/* data float time,  */
#define SM_NWS		(32)		/* wait states; 0=1.5, 1=2.5, ... */
	AT91_SYS->EBI_SMC2_CSR[4] =
		  SM_RWH
		| SM_RWS
		| AT91C_SMC2_ACSS_STANDARD
		| AT91C_SMC2_DBW_16
		// DRP = 0, standard read (not early)
		| AT91C_SMC2_BAT
		// | SM_TDF
		| AT91C_SMC2_WSEN
		| SM_NWS;
...
static int at91_cf_suspend(struct device *dev, pm_message_t mesg, u32 level)
{
...
static int at91_cf_resume(struct device *dev, u32 level)
{
