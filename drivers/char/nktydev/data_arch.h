...
//#define CONFIG_BD//added by duyy, 2012.12.7,用于北大版程序，北大要将消费上限增加到15以上，并且先判断后消费

#define FLOWPERPAGE 25
#ifdef CONFIG_BD
#define SENDLEIDBD 0x14	// 发送光电卡账户信息
#endif
...
#ifdef CONFIG_SUBCNT
#define RECVSBCLEID 0x1B
#endif

/*
 * 修改下载余额命令字，0x1D, modified by zhaozhe, 2011.11.21
 * 下载数据为
 * 1字节账户特征
 * 3字节个人账户余额(BCD)
 * 3字节的实际消费金额(打折或收取管理费后的金额BCD)
 * 校验仍未原来的单字节异或校验
 */
#define RECVLEIDJYB 0x1D	// 教育部修改

#define RECVLEIDGDZJ 0x1E	// 广电总局修改,write by duyy, 2014.1.7
...
#define RECVLEFLOW_ZLYY 0xA0
#define SENDTIME_ZLYY 0x9C
#define SENDLEID_ZLYY3C 0xA1	/* 消费机命令 要求server计算余额,普通消费也支持透支 */
...
typedef struct __acc_ram {		// 应用程序和内核共用
	...
	int m_draw;		/* 可透支钱 */
#endif /* CONFIG_ZLYY */
#ifdef CONFIG_BD//added by duyy for BeiDa, 2012.12.11
	unsigned short consumelimit;  //高字节为禁止消费次数上限，低字节为输密码上限，全1为不限次
	unsigned short consumelife;  //消费次数统计值
#endif
