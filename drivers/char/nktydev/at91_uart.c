...
#if defined EDUREQ
				ret = recv_le_bd(prterm, allow, flag);
#elif defined (S_BID) || defined (USRCFG1) || defined (CONFIG_CNTVER)
				ret = recv_le_bd(prterm, allow, itm);
#else
				ret = recv_le_bd(prterm, allow);
#endif
				break;
#endif
			case SENDRUNDATA:
...
//#endif
			...
			case RECVSBCLEID:
				...
				ret = recv_le_id_sbc(prterm, allow, itm);
				break;
#endif
			case RECVLEBCDFLOW:
				//space_remain--;
			...
			case RECVWATSFLOW:
				ret = recv_water_single_flow(prterm, tm);
				break;
			case RECVWATAFLOW:
				ret = recv_water_order(prterm);
				break;
#endif
#ifdef CONFIG_QH_WATER
			case SENDSYSTIME:
				ret = send_water_time(prterm, tm);
				break;
#endif
			case RECVREFUND:
			...
			case RECVLEIDJYB:
				...
				ret = recv_le_jyb(prterm, allow, 0, itm);
				break;
			case RECVLEIDGDZJ://write by duyy, 2014.1.7
				if ((net_status == 0) && (le_allow == 0)) {
					allow = 0;
				} else {
					allow = 1;
			...
			case SENDLEIDNAME:
				...
				ret = recv_le_id_rdfz(prterm, allow, tm, 1);
				break;
			case RECVDEPFLOWT:
				ret = recv_dep_money_rdfz(prterm, tm, 1);
				break;
			case RECVTAKEFLOWT:
				ret = recv_take_money_rdfz(prterm, tm, 1);
				break;
#endif
...
static int uart_ioctl(struct inode* inode, struct file* file, unsigned int cmd, unsigned int arg)
{
	...
	case SETLAPNUM:
		lapnum = (int)arg;
		break;
	case GETKERNELVSN://获取内核程序版本号，write by duyy, 2014.2.19
		ret = copy_to_user((char *)arg, versn, sizeof(versn));
		if (ret < 0) {
			printk("Uart ioctl GETKERNELVSN copy_to_user error\n");
		} 
		break;
