...
#define DATAFLASH_MAX_DEVICES	1	/* max number of dataflash devices */
 ...
 */
static int dataflash_erase(int pageaddr, int cnt, int flag)
{
...
static int dataflash_read(struct file *file, char *buf, size_t count, loff_t *offp)
{
	...
	spi_access_bus(priv->spi);
	//dataflash_waitready();
	do_spi_trans(2, command, 8, command, 8, rx, count, rx, count);
	spi_release_bus(priv->spi);
...
static int dataflash_open(struct inode *inode, struct file *filp)
{
	...
	switch (dev) {
		case 0:
			...
			spi_ctl->SPI_CSR0 &= 0xFFFFFF;
			spi_ctl->SPI_CSR0 |= DATAFLASH_DLYBCT << 24;
			break;
		case 1:
			spi_csr[dev] = spi_ctl->SPI_CSR1;
			spi_ctl->SPI_CSR1 &= 0xFFFFFF;
			spi_ctl->SPI_CSR1 |= DATAFLASH_DLYBCT << 24;
			break;
		case 2:
			spi_csr[dev] = spi_ctl->SPI_CSR2;
			spi_ctl->SPI_CSR2 &= 0xFFFFFF;
			spi_ctl->SPI_CSR2 |= DATAFLASH_DLYBCT << 24;
			break;
		case 3:
			spi_csr[dev] = spi_ctl->SPI_CSR3;
			spi_ctl->SPI_CSR3 &= 0xFFFFFF;
			spi_ctl->SPI_CSR3 |= DATAFLASH_DLYBCT << 24;
			break;
		default:
			return -EINVAL;
	}
...
static int __init dataflash_init(void)
{
	...
	dflocal = kmalloc(sizeof(struct dataflash_local), GFP_KERNEL);
	if (dflocal == NULL)
		return -ENOMEM;
	...
	printk("DATAFLASH register success!\n");
