 ...
 *
 *  wjzhe: add initial at91_rtc use ds1337 and set the system clock
 */
...
#if 1// defined CONFIG_NKTYDEV21CR || defined CONFIG_CNTVER
extern int getds1337tm(struct rtc_time *rtctm);
extern int set_RTC_time(struct rtc_time *tm, int flag);
static void ds2rtc(void)
{
	struct rtc_time rtctm;
	int err;
	memset(&rtctm, 0, sizeof(rtctm));
	//printk("ds2rtc...read\n");
	if (getds1337tm(&rtctm) < 0) {
		printk(KERN_INFO "AT91 Real Time Clock Get DS1337 Time Failed.\n");
		goto out;
	}
	if (set_RTC_time(&rtctm, 0) < 0) {
		printk(KERN_INFO "AT91 Real Time Clock Set RTC Time Failed.\n");
		goto out;
	}
	...
	if (err == 0) {
		...
		printk(	"setting the system clock to %d-%02d-%02d %02d:%02d:%02d (%u)\n",
	...
	pr_debug("%s(): %4d-%02d-%02d %02d:%02d:%02d wday %02d\n", __FUNCTION__,
		1900 + rtctm.tm_year, rtctm.tm_mon + 1, rtctm.tm_mday,
		rtctm.tm_hour, rtctm.tm_min, rtctm.tm_sec, rtctm.tm_wday);
out:
	return;
}
#endif
...
#if 1// defined CONFIG_NKTYDEV21CR || defined CONFIG_CNTVER
#warning "Read ds1337 and set RTC"
	ds2rtc();
#endif
