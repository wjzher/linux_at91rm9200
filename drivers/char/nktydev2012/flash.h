#ifndef _AT91_FLASH_H
#define _AT91_FLASH_H

#define FLASH_MAJOR 140

/*
 * 64 Mbit -- 64-Kword block, max addr 3fffff
 * 所有FLASH操作都是双字节的, 对于奇数地址要进行额外的处理
 */
// 命令字
...
#define E28FRS	0x70	// read status
...
typedef struct __flash_info_t {
	unsigned long	size;			/* total bank size in bytes	FLASH大小 bytes	*/
	unsigned short	sector_count;		/* number of erase units FLASH块数量 */
	...
	void		   *sect_addr[CFG_MAX_FLASH_SECT];
 ...
 */
#define FLASH_ERASE_TIMEOUT	500000	/* timeout for erasing in 10us		*/
#define FLASH_WRITE_TIMEOUT	200	/* timeout for writes  in 10us		*/
...
#define QRY_IN_CODE		0x98
#define QRY_OUT_CODE	(0x10 << 1)
#define WBCMD			0xE8
#define SRCMD			0x70
#define CONFIRMCMD		0xD0
#define PROGWORD		0x40
#define ERASEFLASH		0x20
...
/* flash status error */
#define ERASESUSPEND	(1 << 6)
#define PROGSUSPEND		(1 << 2)
#define COMSEQERR		(1 << 4)
#define ECLSTATUS		(1 << 5)
#define PROGVOLTAGE		(1 << 3)
#define DEVPROTECT		(1 << 1)
#define WSMSTATUS		(1 << 7)

/* FLASH IOCTL COMMAND */
//_IOWR(type,nr,size)	_IOC(_IOC_READ|_IOC_WRITE,(type),(nr),sizeof(size))
#define FLASH_IOCTL_BASE 'F'
#define ERASEBLOCK 		_IOW(FLASH_IOCTL_BASE, 1, int)
#define SETLOCKSECT		_IOW(FLASH_IOCTL_BASE, 2, int)
#define CLEARLOCKSECT	_IOW(FLASH_IOCTL_BASE, 3, int)
#define GETSECTSIZE		_IOR(FLASH_IOCTL_BASE, 4, int)
#define GETFLASHSIZE	_IOR(FLASH_IOCTL_BASE, 5, int)
#define GETSECTCNT		_IOR(FLASH_IOCTL_BASE, 6, int)
//#define 
#endif
