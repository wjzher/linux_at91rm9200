...
#define RECVDEPFLOWX 0x92	// 接收光电卡存款流水 双校验
#define RECVTAKEFLOWX 0x93	// 接收光电卡取款流水 双校验
#define RECVLEFLOWX 0x94	// 接收光电卡流水 双校验

// 人大附中打小票出纳 打印姓名
#define SENDLEIDNAME 0x95	// 发送存折卡账户信息，增加姓名时间等
#define RECVDEPFLOWT 0x96	// 接收光电卡存款流水 时间 双校验
#define RECVTAKEFLOWT 0x97	// 接收光电卡取款流水 时间 双校验
...
#define SENDLEID_ZLYY2C 0x9E	/* 消费机命令 要求server计算余额 */
#define SENDLEID_ZLYYC 0x9F		/* 出纳机命令 */
...
typedef struct __account {
	...
	unsigned short money_limit;		// 当餐消费上限(0~655.35)
	unsigned char times_limit;		// 当餐消费次数上限(0~255)
	...
	unsigned char flag;				// 标志
									...
									// bit 3: 补贴冻结
									// bit 7..4: 管理费id
	unsigned char st_limit;			// 到达消费上限后的状态
									// bit 0: 个人帐户到达消费上限后允许输密码消费
									// bit 1: 补贴到达消费上限后允许输密码后消费
									// bit 2: 补贴到达消费上限后允许消费个人
									// bit 3: 补贴到达消费上限后输密码消费个人
									// bit 4: 补贴到达消费上限后不允许消费
									// bit 5: 允许跨账户消费
									// bit 6: 优先消费个人金额
									// bit 7: 允许双账户消费
	...
	/* 剩余消费额/次数 */
	unsigned short money_life;		// 个人消费金额
	unsigned char times_life;		// 个人消费次数
	unsigned short smoney_life;		// 补贴消费金额
	unsigned char stimes_life;		// 补贴消费次数

	unsigned char dic_times;		// 打折消费次数统计最大255

	/* 余额报警和透支, wjzhe 20111128 单位元 */
	short warning;				// 报警额度
	unsigned short draft;		// 透支额度
...
/* st_limit bit */
#define ACCST_PWDCON (1 << 0)		// 到达消费上限后允许输密码消费
#define ACCST_PWDCON_SUB (1 << 1)	// 补贴到达上限后允许输密码消费
#define ACCST_PSNL_ALLOW (1 << 2)	// 消费完补贴可消费个人
#define ACCST_PSNL_PWD (1 << 3)		// 补贴到达上限后输密码消费个人
#define ACCST_NO_PSNL (1 << 4)		// 补贴到达上限后不允许消费
#define ACCST_ALLOW_UNIT (1 << 5)	// 允许跨账户消费
#define ACCST_PERSON (1 << 6)		// 优先使用个人账户
#define ACCST_ALLOW_DOUBLE (1 << 7) // 允许双账户消费
...
typedef struct __discount {
	u16 disc_val1;		// 第一次打折比例 0xFFFF无效　0xFFFE禁止消费
	u16 const_val1;		// 第一次固定金额 0xFFFF无效　0xFFFE禁止消费
	u16 disc_val2;		// 第二次打折比例 0xFFFF无效　0xFFFE禁止消费
	u16 const_val2;		// 第二次固定金额 0xFFFF无效　0xFFFE禁止消费
...
struct net_acc
{
	...
	int 	Flag;				//标志（挂失、偷餐）
	int 	ManageId;			//存款管理费，保留（第几个管理费从0开始，
								//同时提供管理费系数列表char[n]）
								//最高位表示是否允许存款
	char 	Password[4];		//消费密码				
	int 	UpLimitMoney;		//总消费上限
	unsigned char	UpLimitTime;	//总消费次数上限
	unsigned char	FreezeFlag; 	//冻结标志
	unsigned char	PowerId;			//存折卡余额身份类型 1~16
	unsigned char	PwdType;		//到达消费上限是否允许输密码消费
							//bit0=0表示到达消费上限后不可用
							//bit0=1表示到达消费上限后输密码可继续消费
							// bit 5 标志是否允许跨账户消费
							// bit6 1 允许消费双账户
							// bit6 0 只允许消费单账户
							// bit7=0 表示优先消费补贴金额
	...
	unsigned char	SubPwdType; //到达消费上限后状态，
							//0表示到达消费上限后不可用
							//1表示到达消费上限后输密码继续消费补贴额
							//2表示到达消费上限后输密码继续消费个人余额
							//3表示到达消费上限后不输入密码继续消费个人余额
	int 		ConsumedLimitedMoney; 	//剩余允许本餐已消费总消费金额
	unsigned short	SubConsumedLimitedMoney;	//剩余允许本餐补贴账户已消费补贴消费金额
	unsigned char	ConsumedLimitedTimes; 	//本餐已消费剩余允许消费次数
