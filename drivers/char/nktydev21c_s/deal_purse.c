 ...
 */
int compute_blk(term_ram *ptrm, struct black_info *blkinfo)
{
	//int ret;
	// 判断是否存在正确的交易号和版本号
	...
	if (ptrm->blkinfo.edition != blkinfo->edition) {
		printk("compute_blk term %d blk edition not match %d: %d\n",
			ptrm->term_no, ptrm->blkinfo.edition, blkinfo->edition);
		//return -1;
	}
#endif
#if 0
...
#endif
	// 必须保证交易号连续, 此返回值才正确
	//printk("blk %d\n", blkinfo->trade_num.num - ptrm->blkinfo.trade_num.num);
	return blkinfo->trade_num.num - ptrm->blkinfo.trade_num.num;
}
