...
flow pflow[FLOWANUM];
int maxflowno = 0;
unsigned char pfbdseg[28] = {0};//add by duyy,2013.7.1
unsigned char ptmnum[16][16];//add by duyy,2013.7.1,timenum[身份][终端]
term_tmsg term_time[7];//write by duyy, 2013.7.1
...
#define TM3E (20 * 3600 + 0 * 60)
			...
			case PURSE_RECV_CARDNO:
				...
				ret = purse_recv_leid(prterm, allow, tm);//for modified a debug,&tm->tm,2013.3.26
				break;
...
//		printk("le allow %d, %d\n", le_allow, ptr.le_allow);
		...
		break;
	case SETFORBIDTIME://初始化终端禁止消费时段,added by duyy, 2013.7.1
...
#endif
		break;
	case SETTIMENUM:		// 设置身份、终端类型匹配时段序号,added by duyy, 2013.7.1
	...
	case GETCASHBUF:
		//if (cashterm_ptr) {
			...
			if (ret < 0) {
				...
				ret = CASHBUFSZ;//cashterm_ptr;//modified by duyy, 2013.7.1
			}
