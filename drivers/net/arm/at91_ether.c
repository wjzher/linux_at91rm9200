...
static int update_linkspeed(struct net_device *dev, AT91PS_EMAC regs) {
	...
	if (bmcr & BMCR_ANENABLE) {				/* AutoNegotiation is enabled */
		if (!(bmsr & BMSR_ANEGCOMPLETE)) {
			if (lp->phy_type == MII_DM9161A_ID) {
				int i = 0;
				while (!(bmsr & BMSR_ANEGCOMPLETE)) {
					read_phy(regs, lp->phy_address, MII_BMSR, &bmsr);
					i++;
					if (i > 40000) {	// 如果40000都无法得到ANEGCOMPLETE
						printk("read bmsr %d ANEGCOMPLETE\n", i);
						goto err;		// 网络断
					}
#ifdef ETHDEBUG
					printk("read mii bmsr...\n");
#endif
				}
				goto end;
			}
 ...
 * ethtool support.
 */
#warning "Convert to ethtool_ops"
...
static int at91ether_open(struct net_device *dev)
{
	...
	spin_lock_irq(&lp->lock);
	enable_mdi(regs);
	link = update_linkspeed(dev, regs);
...
static int __init at91ether_setup(unsigned long phy_type, unsigned short phy_address, struct at91_eth_data *board_data)
{
	...
	spin_lock_irq(&lp->lock);
	enable_mdi(regs);
	if (phy_type == MII_DM9161_ID || phy_type == MII_DM9161A_ID) {
	...
	else if (phy_type == MII_DM9161A_ID)
		printk(KERN_INFO "%s: Davicom 9196A PHY %s\n", dev->name, (lp->phy_media == PORT_FIBRE) ? "(Fiber)" : "(Copper)");
...
static int __init at91ether_probe(struct device *dev)
{
	...
	while ((detected != 0) && (phy_address < 32)) {
		...
		phy_id = (phyid1 << 16) | (phyid2 & 0xfff0);
                printk("addr %d, id %08x\n", phy_address, phy_id);
		switch (phy_id) {
			case MII_DM9161_ID:		/* Davicom 9161: PHY_ID1 = 0x181, PHY_ID2 = B881 */
			case MII_DM9161A_ID:
                        case MII_DM9161_ID_ERR:
